...
const NewCreateOrder = () => {
	...
	const fetchMenuData = async () => {
		try {
			...
			const [menuCategoryRes, menuRes] = await Promise.all([
	...
	const getOrderNumber = async () => {
		try {
			...
			res.data.forEach((order) => {
				if (order.orderNumber > nextNumber) {
					nextNumber = order.orderNumber;
				}
			});
			nextNumber += 1;
	...
	const saveOrder = async () => {
		try {
			...
			const res = await axiosPrivate.post("order/createOrder", newOrder);
				...
				if (menu.recipeId) {
					...
					recipeRes.data.stockUsed.map(async (stock) => {
						const newStockMovement = {
							...
							movementType: "Order",
							orderId: orderRes.data.object._id,
							employeeId: auth.userId,
						};
