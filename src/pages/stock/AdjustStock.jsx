		...
		try {
			...
			setSelectedStock(
				stockRes.data.map((stock) => ({
					...
					prevQuantity: stockRecapRes.data.find(
						(stock2) => stock2._id === stock._id
					).quantity,
					quantity: stockRecapRes.data.find(
						(stock2) => stock2._id === stock._id
					).quantity,
				}))
			);
	...
	const saveStockAdjustment = async () => {
		try {
			...
			selectedStock.forEach(async (stock) => {
				if (stock.quantity !== stock.prevQuantity) {
					const newStockMovement = {
						stockId: stock.stockId,
						stockQuantityChange:
							stock.quantity - stock.prevQuantity,
